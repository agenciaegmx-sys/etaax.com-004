...
// Migración única: la sesión antes vivía en sessionStorage. Si quedó ahí y
// localStorage aún no la tiene, la copiamos para NO forzar re-login al cambiar
// el almacenamiento a localStorage.
try {
    for (var _i = 0; _i < sessionStorage.length; _i++) {
        var _k = sessionStorage.key(_i);
        if (_k && _k.indexOf('sb-') === 0 && !localStorage.getItem(_k)) {
            localStorage.setItem(_k, sessionStorage.getItem(_k));
        }
    }
} catch (e) {}
